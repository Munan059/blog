...
预训练阶段需要获取大量可用于训练的文本数据。

![](/articles/karpathy-llm-notes/karpathy-20260802150351.png)

- 预训练阶段首先要过滤url确保恶意网站和一些营销网站等不被放入训练数据中
- 其次要进行文本类型的过滤，例如你在访问html网页的时候你需要把其中的内容训练成一个数据集那你就需要访问它的源代码，里面就会有很多的标签或者css你需要把这些剔除使它成为一个只有文字的正常数据
- 之后是语言过滤你可以选择你需要训练对应语言的模型你就要需要对应语言的数据比如英语西班牙语中文等

## 分词（Tokenization）

![](/articles/karpathy-llm-notes/karpathy-20260802170527.png)

英文模型通常将一个常见英文单词算作一个token，但实际上很多单词会被拆成多个token，每个token有对应的数字来指代它，并且以此为基础进行分词。

你可以将约5万个数字中间以逗号分割，配合合并规则，以此形成一个分词器。

## 神经网络与 Transformer

![](/articles/karpathy-llm-notes/karpathy-20260802164116.png)

当你把几个单词放进神经网络里，他会给你预测接下来什么单词出现的概率会更高，这通常依赖你提供给他的上下文，并且你可以通过更新神经网络来确保出现正确单词的可能性更高，这就是训练神经网络的过程。

![](/articles/karpathy-llm-notes/karpathy-20260802164825.png)

将你的输入的一些单词表示为x并且将多个参数也叫做权重共同放进一个非常大的数学表达式它最终能得出一个预测结果

![](/articles/karpathy-llm-notes/karpathy-20260802165301.png)

这个特殊的神经网络叫做transformer，将输入的单词通过一系列的数学函数最终转化为一个预测的输出，并且可以通过不同参数的调整来得到不同的输出。

![](/articles/karpathy-llm-notes/karpathy-20260802170635.png)

通过一些输入，模型可以通过推理来预测91所对应的单词通过神经网络推理可能性得到了一个范例，后面是什么数字所对应的单词，以此类推，得到的结果可能由于概率学的原因会和正确的结果有所偏差你需要调整神经网络的权重来保证它得到正确的结果。

## 典型模型参数（GPT-2 与 Llama 3）

![](/articles/karpathy-llm-notes/karpathy-20260808135951.png)

以gpt-2了解模型的一些术语，它用了约15亿的参数、大约100亿训练数据、最大的token数为1024 token。

![](/articles/karpathy-llm-notes/karpathy-20260808141149.png)

同样的，Llama3的参量为4050亿、用于训练的数据为15万亿

## 基础模型的本质

![](/articles/karpathy-llm-notes/karpathy-20260808141416.png)

基础模型本质上是一个提前训练好的互联网文档模拟器，它有一定的幻觉率，它的参量是有损压缩、且是具备随机性的、它主要通过一些记忆来复述一些记忆文档。

# 后训练阶段

## 用对话数据做训练

![](/articles/karpathy-llm-notes/karpathy-20260808144635.png)

大语言模型的后训练阶段主要是将对话作为数据进行训练，替换了互联网的数据集作为一个新的数据集进行训练。模型通过学习人类对话给出的具体回答的表现进行训练，耗时通常比预训练阶段要少得多。

## 对话数据集是怎么来的

![](/articles/karpathy-llm-notes/karpathy-20260808152716.png)

这张图展示了一个对话数据集，展示了从人工标注员根据标注规则来写对话，到LLM辅助或者纯LLM进行对话数据的实现。

## 模型幻觉

![](/articles/karpathy-llm-notes/karpathy-20260808145838.png)

模型幻觉：指的是模型内无法搜索到相关信息，从而自己编造莫须有的事实。其本质上不是在查找信息，是在模仿信息。

## 应对未知问题与 Function Call

![](/articles/karpathy-llm-notes/karpathy-20260808153656.png)

当模型回答不出一些问题的时候，你可以训练它让它回答不知道，这是其中一种方式。而另一种方式是Function call，你可以训练它通过浏览器的搜索功能查找答案，它通过搜索功能将搜索到的答案记录到上下文窗口，以便他能够通过上下文窗口了解到他搜索到的答案。这也是需要通过一个搜索功能的一个专用数据集来实现的。

## 如何让模型记住自己的身份

![](/articles/karpathy-llm-notes/karpathy-20260808160836.png)
...
![](/articles/karpathy-llm-notes/karpathy-20260808160907.png)

左右两侧答案都是正确的答案，但左边的答案明显不如右边的好，左边的答案直接给出了结果，后续才给出推理过程，而右边的答案是一步一步推导符合大模型的思考过程。这是因为大模型的思考过程本质上是从左到右的一个一维数组，而左边在给出正确答案后，剩下的部分都是在给正确答案做解释，实际上没有在计算。这只是尝试在单个token中猜测答案，这是不可取的做法，因为每个token的计算量是有限的。

右边的答案将计算的算力分布在答案上，让模型一步步得出答案。

![](/articles/karpathy-llm-notes/karpathy-20260808164454.png)

由于单词被分词器划分成各个token，这使得模型在计数和拼写方面表现得很差。但模型可以使用一些工具来进行弥补，比如：使用浏览器或者编码工具等。

## SFT 监督微调

**SFT(Supervised Fine-Tuning)监督微调**:在训练完基础模型以后,模型已经获得了许多知识,这个时候拿大量的问答案例给它参考，让它学习怎么答题。

# 强化学习阶段

## 用"读书"理解三个阶段

![](/articles/karpathy-llm-notes/karpathy-20260808165439.png)

以一本书为例子，把蓝色部分的背景知识比作预训练阶段，这个阶段你获取大量知识。

然后红色部分的问题和参考答案共同构成了后训练阶段，这个阶段你学习了具体的问题和答案。

而绿色部分的强化学习阶段，则更像是给你提出了问题但没有解决方案，此时，你需要通过背景知识和之前的问题和解答中获取到的知识对这个问题进行解答，这个时候就需要自己进行大量的尝试来找到正确答案，这就是强化学习的目的。

## 试错与择优

![](/articles/karpathy-llm-notes/karpathy-20260808212701.png)

当你的模型在尝试的过程中，会给出多种解决方案，而只有其中的几种是正确的回答，模型需要找到其中最好的那个，然后用于训练。

## RLHF：用人类偏好当奖励模型

![](/articles/karpathy-llm-notes/karpathy-20260808215357.png)

原始方法：每次训练模型，都需要人类给模型生成的回答打分。这会带来非常昂贵的成本，所以引出下面的方法。

RLHF方法：先让人类进行偏好选择，从最好到最差进行排序，然后用人类偏好训练出一个奖励模型，以此使用奖励模型来代替真人进行训练。

## RLHF 的优势

![](/articles/karpathy-llm-notes/karpathy-20260808221356.png)

RLHF的优势用一句话概括就是，RLHF让强化学习可以扩展到"没有标准答案"的任务上。

## RLHF 的缺点

![](/articles/karpathy-llm-notes/karpathy-20260808221929.png)

RLHF的缺点是，RLHF 的奖励模型只是人类偏好的一个有损压缩版，而强化学习会利用这个其中的漏洞，可能无法达到想要实现的目标。`,
};
